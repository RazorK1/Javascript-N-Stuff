...
// Declare the thisDate variable containing the Date object or leave empty for today's date
// Change this value to test different dates as needed
let thisDate = new Date(); 

// Declare the dateString variable to store the local date as a string
let dateString = thisDate.toLocaleDateString(); // Format date using local conventions

// Declare the dateHTML variable containing the formatted date string as an <h2> element
let dateHTML = `<h2>${dateString}</h2>`;

// Get the day of the week number using the getDay() method
let thisDay = thisDate.getDay(); // Sunday = 0, Monday = 1, ..., Saturday = 6

// Use the getEvent() function to get the HTML code for today's events
let eventHTML = getEvent(thisDay);

// Insert the dateHTML and eventHTML into the element with ID "unionToday"
